...
test('mapPostingToOrder maps analytics and financial data into order totals', () => {
  ...
  assert.equal(mapped.order.currency, 'CNY');
  ...
  assert.deepEqual(mapped.order.remark.currency_context, {
    original: 'RUB',
    target: 'CNY',
    rate: 1
  });
  ...
  assert.equal(mapped.items[0].currency, 'CNY');
  assert.equal(mapped.items[1].sku, 'SKU-2');
});

test('mapPostingToOrder converts posting totals using currency_rate_to_cny', () => {
  const posting = {
    posting_number: 'OZ-1002',
    status: 'delivered',
    created_at: '2024-09-05T08:30:00Z',
    analytics_data: {
      revenue: 150,
      currency_code: 'RUB',
      currency_rate_to_cny: 0.1,
      delivery_amount: 12
    },
    financial_data: {
      currency: 'RUB',
      posting_is_paid: true,
      posting_services: {
        marketplace_service_item_fulfillment: 8
      },
      products: [
        { sku: 'SKU-3', name: 'Item 3', quantity: 3, price: 20, total_price: 60, cost_price: 8 },
        { sku: 'SKU-4', name: 'Item 4', quantity: 2, price: 30, total_price: 60, cost_price: 12 }
      ]
    }
  };
  ...
  const mapped = mapPostingToOrder(posting, { siteId: 'ozon_main', type: 'fbs' });
  assert.equal(mapped.order.currency, 'CNY');
  assert.equal(mapped.order.subtotal, 12); // (60 + 60) * 0.1
  assert.equal(mapped.order.total, 15); // 150 * 0.1
  assert.equal(mapped.order.shipping_fee, 1.2); // 12 * 0.1
  assert.equal(mapped.order.logistics_cost, 0.8); // 8 * 0.1
  assert.equal(mapped.order.cost_of_goods, 4.8); // (8*3 + 12*2) * 0.1
  assert.deepEqual(mapped.order.remark.currency_context, {
    original: 'RUB',
    target: 'CNY',
    rate: 0.1
...
test('aggregatePostings merges duplicate postings and concatenates items', () => {
  ...
  assert.equal(order.remark.currency_context.original, 'RUB');

...
CREATE TABLE IF NOT EXISTS public.site_channel_configs (
...
-- 0.b Lazada/Shopee 等授权凭据（仅服务端访问）
...
CREATE INDEX IF NOT EXISTS idx_integration_tokens_site_provider ON public.integration_tokens(site_id, provider);
...
CREATE INDEX IF NOT EXISTS idx_product_metrics_daily_site_date ON public.product_metrics_daily(site, stat_date);
CREATE INDEX IF NOT EXISTS idx_product_metrics_daily_sku ON public.product_metrics_daily(sku);
...
CREATE POLICY p_product_metrics_all ON public.product_metrics_daily FOR ALL TO anon USING (true);
CREATE POLICY p_integration_tokens_service ON public.integration_tokens FOR ALL TO service_role USING (true) WITH CHECK (true);

...
function createHandler({ fetchImpl = fetch, clientFactory = createClient, syncAds = syncLazadaAds } = {}) {
  return async function handler(req, res) {
    const traceId = `lazada-ads-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    try {
      ...
      console.error(`[${traceId}] Lazada ads handler failed`, error);
      ...
      const payload = {
        success: false,
        message: error.message || 'Lazada ads sync failed',
        traceId
      };

...
export default async function handler(req, res) {
  ...
  try {
    ...
    const crossSiteIds = new Set();
    for (const row of rows) {
      const prev = siteMap.get(row.product_id);
      if (prev && prev !== row.site) {
        crossSiteIds.add(row.product_id);
      } else {
        siteMap.set(row.product_id, row.site);
      }
    }
    if (crossSiteIds.size > 0) {
      return res.status(200).json({ ok: false, error: '数据表格站点有误。', product_ids: [...crossSiteIds] });
    }
    ...
    if (crossSite.length > 0) {
      return res.status(200).json({ ok: false, error: '数据表格站点有误。', product_ids: [...new Set(crossSite)] });
    }
    ...
    return res.status(200).json({ ok: true, message: `上传完成，成功上传${inserted}组数据`, upserted: inserted });
  } catch (e) {
    // 任何异常都返回 JSON

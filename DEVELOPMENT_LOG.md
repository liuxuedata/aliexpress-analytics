...
**功能描述**: 实现全局管理功能，包含库存管理、权限管理、站点配置三个核心模块

**实现内容**:
1. **修改admin页面架构**:
   - 将admin页面改为专注于三个全局模块
   - 更新侧边栏菜单：只保留库存管理、权限管理、站点配置
   - 实现模块间切换逻辑
   - 更新页面标题动态显示

2. **在所有站点页面添加全局管理入口**:
   - 自运营站点：在用户下拉菜单中添加"全局管理"选项
   - 全托管站点：在用户下拉菜单中添加"全局管理"选项
   - 独立站：在用户下拉菜单中添加"全局管理"选项
   - 亚马逊站点：在顶部导航添加全局管理按钮
   - Ozon站点：在顶部导航添加全局管理按钮

3. **三个全局模块设计**:
   - **📦 库存管理**: 全局产品和库存管理，包含KPI统计、筛选器、数据表格
   - **👥 权限管理**: 用户和角色权限管理，包含用户统计、角色管理、权限分配
   - **⚙️ 站点配置**: 全局站点配置和模块设置，包含站点统计、平台管理、模块配置

**技术实现**:
- 更新JavaScript导航逻辑，支持模块间切换
- 添加事件处理，实现全局管理按钮跳转
- 优化用户体验，统一全局管理入口

**文件变更**:
- ✅ 修改 `public/admin/index.html` - 重构为全局管理页面
- ✅ 更新 `public/self-operated.html` - 添加全局管理入口
- ✅ 更新 `public/managed.html` - 添加全局管理入口
- ✅ 更新 `public/independent-site.html` - 添加全局管理入口
- ✅ 更新 `public/amazon-overview.html` - 添加全局管理按钮
- ✅ 更新 `public/ozon-detail.html` - 添加全局管理按钮

**提交信息**: `feat: 实现全局管理功能，包含库存管理、权限管理、站点配置三个模块`

### 全局管理页面问题修复

**问题描述**: 全局管理页面存在多个问题
1. DataTables Ajax错误，表格ID不匹配
2. 权限管理页面空白，没有显示内容
3. 站点配置页面空白，需要集成现有功能

**问题分析**:
1. 权限管理模块的容器ID是`users-container`，但导航中引用的是`permissions`
2. 缺少站点配置模块的HTML内容
3. DataTables初始化时没有正确的错误处理
4. API调用失败时没有降级处理

**修复方案**:
1. 修复权限管理模块ID：`users-container` → `permissions-container`
2. 添加完整的站点配置模块内容，包含表格和筛选器
3. 完善DataTables错误处理，确保API失败时显示空表格
4. 添加测试数据生成功能，方便开发测试
5. 完善JavaScript模块初始化逻辑

**文件变更**:
- ✅ 修复 `public/admin/index.html` - 修复模块ID和添加站点配置内容
- ✅ 完善DataTables错误处理逻辑
- ✅ 添加测试数据生成功能

**技术改进**:
- 所有表格都支持错误处理，在没有数据时显示空表格而不是报错
- 添加了测试数据生成按钮，方便开发和测试
- 完善了模块间切换逻辑

...
### Lazada 数据管道
- `GET /api/lazada/oauth/callback`：作为 Lazada 授权回调，接受 `code`、`state` 或 `error` 查询参数，使用 `LAZADA_APP_KEY/LAZADA_APP_SECRET/LAZADA_REDIRECT_URI` 计算签名并向 Lazada 授权服务器换取访问令牌；成功后会尝试解析 `state` 中的 `siteId` 并将访问/刷新令牌写入 `integration_tokens` 表，避免落在前端。【F:api/lazada/oauth/callback.js†L1-L192】【F:specs/data-model.sql†L29-L52】
...
## 环境配置
- **Lazada**：在 Vercel 环境变量中配置 `LAZADA_APP_KEY`、`LAZADA_APP_SECRET`、`LAZADA_REDIRECT_URI`、`SUPABASE_URL` 与 `SUPABASE_SERVICE_ROLE_KEY`，其中回调地址固定为 `https://aliexpress-analytics.vercel.app/api/lazada/oauth/callback`。授权完成后，后端会将访问/刷新令牌写入 `integration_tokens` 表，API 再基于刷新令牌自动换取短期访问令牌并落地运营、订单、广告数据，前端无需接触敏感凭据。【F:api/lazada/oauth/callback.js†L1-L192】【F:lib/lazada-auth.js†L1-L120】【F:specs/data-model.sql†L29-L52】

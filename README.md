...
| `ozon-*.html` | Ozon 报表上传与多指标分析 | 明细/运营/产品页面均补充订单与广告入口，并新增 `ozon-orders.html`、`ozon-advertising.html`，二者直接对接 Ozon 官方 API 同步订单与广告数据 | `/api/ozon/stats`、`/api/ozon/import`（运营报表上传）、`/api/ozon/orders`、`/api/ozon/ads`（规划中）【F:public/ozon-detail.html†L40-L78】【F:public/ozon-orders.html†L1-L68】【F:public/ozon-advertising.html†L1-L60】 |
...
- **Ozon 页面集**：`public/ozon-detail.html` 等页面提供上传入口、日期筛选及多图表分栏；`ozon-orders.html`、`ozon-advertising.html` 负责直连 Ozon 官方 API 同步订单、广告系列与日指标，服务层以 `ozon_api` 数据源写入统一订单/广告表结构。【F:public/ozon-detail.html†L40-L78】【F:public/ozon-orders.html†L1-L68】【F:public/ozon-advertising.html†L1-L60】【F:rules.json†L4-L55】
...
- **Ozon**：`site_configs` 中的数据源需配置为 `ozon_api`，后台定时作业直接调用 Ozon 官方订单与广告 API，将结果写入统一的 `orders`、`order_items`、`ad_campaigns`、`ad_metrics_daily` 表，左侧导航由 `site_module_configs.has_data_source` 控制显隐。【F:public/ozon-orders.html†L1-L68】【F:public/ozon-advertising.html†L1-L60】【F:rules.json†L4-L55】
...
- `GET /api/lazada/oauth/callback`：接收 Lazada OAuth 回调 `code`、`state`、`error` 参数并返回标准 JSON 响应，便于服务端换取访问令牌；默认回调地址与 Vercel 环境变量保持一致。【F:api/lazada/oauth/callback.js†L1-L63】
...
- `GET /api/ozon/orders`（规划中）：作为订单中心的数据源，直接调取 Ozon 官方订单 API，按站点、时间范围同步订单头、明细与物流成本字段并写入统一订单表；响应会补充 `metadata.availableFields` 供前端判定缺失字段。【F:public/ozon-orders.html†L1-L68】【F:specs/openapi.yaml†L1200-L1305】
- `GET /api/ozon/ads`（规划中）：调用 Ozon 广告 API 获取广告系列、日预算与投放指标，映射至 `ad_campaigns`、`ad_metrics_daily` 表，并支持站点、日期、campaign 多条件过滤。【F:public/ozon-advertising.html†L1-L60】【F:specs/openapi.yaml†L1306-L1388】

### Lazada OAuth 配置

- **环境变量**：在 Vercel 环境变量面板中配置 `LAZADA_APP_KEY`、`LAZADA_APP_SECRET`、`LAZADA_REDIRECT_URI`（默认为 `https://aliexpress-analytics.vercel.app/api/lazada/oauth/callback`），按环境区分可分别填写预览与生产值。【F:api/lazada/oauth/callback.js†L33-L55】
- **授权流程**：Lazada 完成授权后回调 `/api/lazada/oauth/callback?code=...&state=...`；服务端使用返回的 `code` 调用 Lazada Access Token 接口换取令牌，并将密钥保存到安全的密钥存储（如 Vercel 加密变量或 Supabase Vault），严禁写入 `site_configs.config_json`。【F:api/lazada/oauth/callback.js†L1-L63】【F:rules.json†L4-L54】

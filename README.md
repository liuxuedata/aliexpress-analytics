...
| `site-dashboard.html` | 站点控制台，为 Lazada/Shopee 等新平台站点提供动态模块框架 | 顶部复用全局导航，左侧根据 `/api/site-modules` 生成锚点并展示字段/配置占位 | `/api/site-configs`、`/api/site-modules/{siteId}`【F:public/site-dashboard.html†L338-L752】 |

- **入口页 `public/index.html`**：作为平台门户，内置渐变过渡和加载动画，并在 1 秒内自动重定向到自运营 Robot 站，确保默认落地页一致。【F:public/index.html†L1-L58】
- **自运营页 `public/self-operated.html`**：聚合 DataTables、ECharts、Flatpickr 等库，侧边栏固定包含“详细数据/运营分析/产品分析/订单中心/广告中心”五大模块；订单与广告板块暂提供占位说明，默认站点包含 Robot 与 Poolslab，可在导航中快速切换。【F:public/self-operated.html†L520-L788】【F:public/assets/site-nav.js†L145-L219】
...
- **站点控制台 `public/site-dashboard.html`**：用于 Lazada、Shopee 等新站点的统一壳层，页面启动后读取 `/api/site-configs` 与 `/api/site-modules/{siteId}` 动态生成左侧模块导航与模块占位卡片，并输出字段覆盖与自定义配置提示。【F:public/site-dashboard.html†L338-L752】
...
- **共享导航脚本 `public/assets/site-nav.js`**：初始化时从 `/api/site-configs` 读取站点列表，自动填充速卖通与独立站下拉菜单，并为 Lazada、Shopee 等新平台生成顶栏入口，点击后写入本地存储并可跳转至动态控制台。【F:public/assets/site-nav.js†L111-L220】【F:public/assets/site-nav.js†L268-L586】
...
### 站点与渠道分类
- **速卖通自运营**：默认提供 Robot 站（`ae_self_operated_a`）与 Poolslab 站（`ae_self_operated_poolslab_store`），可通过站点选择器和 `localStorage` 记忆切换。【F:public/assets/site-nav.js†L145-L219】
- **速卖通全托管**：通过顶部“速卖通 → 全托管”下拉菜单加载站点列表，配合上传控件与多图表分析。【F:public/managed.html†L9-L122】
- **独立站**：支持 Facebook、Google、TikTok 渠道，站点默认包含 `poolsvacuum.com` 与 `icyberite.com`，并在导航下拉与页面内同步显示。【F:public/assets/site-nav.js†L222-L265】
- **多平台站点**：共享导航脚本会读取 `site-configs` 将 Lazada、Shopee 等平台插入顶栏并指向 `site-dashboard.html`，新站点上线后无需手动改导航即可访问模块框架。【F:public/assets/site-nav.js†L268-L586】【F:public/site-dashboard.html†L338-L752】

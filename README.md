...
运行 `migration_add_sites.sql` 可在数据库中创建统一的 `sites` 表。

新的 `/api/sites` 接口提供站点列表获取与新增能力：

- `GET /api/sites` 返回已有站点数组。
- `POST /api/sites` 携带 `{ name: '站点名' }` 可新增站点。

前端页面会在加载时请求站点列表，并支持通过界面按钮快速创建新站点。

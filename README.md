...
### Lazada 数据管道
- `GET /api/lazada/oauth/start`：生成 Lazada 授权地址，校验站点是否属于 Lazada，并在 `state` 中签名保存 `siteId/returnTo` 等上下文信息，前端点击“立即授权”按钮时会调用该接口并跳转 Lazada 登录页。【F:api/lazada/oauth/start.js†L1-L144】【F:public/lazada.html†L1-L520】
...
## 环境配置
- **Lazada**：在 Vercel 环境变量中配置 `LAZADA_APP_KEY`、`LAZADA_APP_SECRET`、`LAZADA_REDIRECT_URI`、`SUPABASE_URL` 与 `SUPABASE_SERVICE_ROLE_KEY`，其中回调地址固定为 `https://aliexpress-analytics.vercel.app/api/lazada/oauth/callback`。授权入口通过 Lazada 页面“立即授权”按钮触发 `/api/lazada/oauth/start`，成功回调后 API 会自动重定向回原页面并刷新数据；若返回 `TOKEN_NOT_FOUND`，可再次点击按钮重新授权。【F:api/lazada/oauth/start.js†L1-L144】【F:api/lazada/oauth/callback.js†L1-L220】【F:public/lazada.html†L1-L520】

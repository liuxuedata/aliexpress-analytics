...
async function syncLazadaStats({
  ...
  const platform = siteConfig.platform || 'lazada';
  if (shouldSync) {
    ...
    const mappedDaily = (dailyRecords || [])
      .map(record => mapDailyRow(record, { siteId: normalizedSiteId, platform }))
      .filter(Boolean)
      .map(row => ({ ...row, platform }));

    const mappedProducts = (productRecords || [])
      .map(record => mapProductRow(record, { siteId: normalizedSiteId, platform }))
      .filter(Boolean)
      .map(row => ({ ...row, platform }));
  ...
  summary.siteId = normalizedSiteId;

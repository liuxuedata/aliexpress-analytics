...
function convertAmount(value, rate = 1) {
  ...
  return toCurrency(numeric * multiplier);
}
...
function resolveConversionRate(posting, originalCurrency, targetCurrency = 'CNY') {
  ...
  const rateCandidates = [
    posting?.analytics_data?.[`currency_rate_to_${lowerTarget}`],
    posting?.analytics_data?.currency_rate_to_cny,
    posting?.analytics_data?.currency_rate,
    posting?.analytics_data?.exchange_rate,
    posting?.analytics_data?.rate,
    posting?.financial_data?.[`currency_rate_to_${lowerTarget}`],
    posting?.financial_data?.currency_rate_to_cny,
    posting?.financial_data?.currency_rate,
    posting?.financial_data?.exchange_rate,
    posting?.financial_data?.rate
  ];

  const resolved = pickFirstNumber(rateCandidates, 0);
  if (resolved && resolved > 0) {
    return resolved;
  }

  const envKey = `OZON_${originalCurrency.toUpperCase()}_TO_${targetCurrency.toUpperCase()}_RATE`;
  const envRate = toNumber(process.env[envKey], 0);
...
function resolveCurrency(posting, products) {
  ...
  let sourceCurrency = analyticsCurrency;
  for (const product of products) {
    if (product?.currency_code) {
      sourceCurrency = product.currency_code;
      break;
    }
    if (product?.price_data?.currency) {
      sourceCurrency = product.price_data.currency;
      break;
    }
  }
  if (!sourceCurrency && posting?.financial_data?.currency) {
    sourceCurrency = posting.financial_data.currency;
  }

  const normalizedSource = typeof sourceCurrency === 'string'
    ? sourceCurrency.trim().toUpperCase()
    : '';

  const currency = 'CNY';
  const rate = resolveConversionRate(posting, normalizedSource || null, currency);

  return {
    currency,
    originalCurrency: normalizedSource || currency,
    rate
  };
}

function mapProductToOrderItem(product, index, currencyInfo) {
  ...
  const unitPriceRaw = pickFirstNumber(priceCandidates, product?.result_price || product?.sale_price || 0);
  ...
  const totalPriceRaw = pickFirstNumber(totalPriceCandidates, unitPriceRaw * quantity);

  const costPriceRaw = pickFirstNumber([
    product?.cost_price,
  ...
  const currency = currencyInfo?.currency || 'CNY';
  const unitPrice = convertAmount(unitPriceRaw, rate);
  const totalPrice = convertAmount(totalPriceRaw, rate);
  const costPrice = Number.isFinite(toNumber(costPriceRaw)) ? convertAmount(costPriceRaw, rate) : null;
...
function mapPostingToOrder(posting, { siteId, type }) {
  ...
  const currencyInfo = resolveCurrency(posting, products);
  const items = products.map((product, index) => mapProductToOrderItem(product, index, currencyInfo));

  const subtotal = toCurrency(items.reduce((sum, item) => sum + toNumber(item.unit_price) * item.quantity, 0));
  const rawDiscount = products.reduce((sum, product) => sum + toNumber(
    pickFirstNumber([
  ...
  const discount = convertAmount(rawDiscount, currencyInfo.rate);
  const rawTax = products.reduce((sum, product) => sum + toNumber(
    pickFirstNumber([
      product?.vat,
      product?.tax_amount,
      product?.tax_sum
  ...
  const tax = convertAmount(rawTax, currencyInfo.rate);
  const costOfGoods = toCurrency(items.reduce((sum, item) => sum + toNumber(item.cost_price || 0) * item.quantity, 0));
  const rawAnalyticsTotal = pickFirstNumber([
    posting?.analytics_data?.revenue,
    posting?.analytics_data?.ordered_amount
  ], null);
  const analyticsTotal = rawAnalyticsTotal !== null && rawAnalyticsTotal !== undefined
    ? convertAmount(rawAnalyticsTotal, currencyInfo.rate)
    : toCurrency(subtotal - discount);
  const rawShipping = pickFirstNumber([
    posting?.analytics_data?.delivery_amount,
    posting?.financial_data?.posting_services?.marketplace_service_item_delivery
  ]);
  const shippingFee = convertAmount(Math.abs(rawShipping), currencyInfo.rate);
  const logisticsCost = convertAmount(Math.abs(sumPostingServices(posting?.financial_data?.posting_services)), currencyInfo.rate);
  ...
  const remark = {
    ...
    currency_context: {
      original: currencyInfo.originalCurrency,
      target: currencyInfo.currency,

...
function wrapIntegrationTokenError(action, supabaseError) {
  if (!supabaseError) return null;
  const message = supabaseError.message || '未知错误';
  if (message.includes('integration_tokens')) {
    const err = new Error(
      `Supabase ${action} Lazada 凭据失败：缺少 integration_tokens 表。请执行 create_integration_tokens_table.sql 迁移脚本后重试。原始错误：${message}`
    );
    err.code = 'INTEGRATION_TOKENS_TABLE_MISSING';
    err.cause = supabaseError;
    return err;
  }

  const err = new Error(`Supabase ${action} Lazada 凭据失败：${message}`);
  err.cause = supabaseError;
  return err;
}
...
async function loadTokenRecord(supabase, siteId, provider = PROVIDER) {
  ...
  if (error) {
    throw wrapIntegrationTokenError('查询', error);
  }
...
async function storeTokenRecord(supabase, {
  ...
  if (error) {
    throw wrapIntegrationTokenError('写入', error);
  }

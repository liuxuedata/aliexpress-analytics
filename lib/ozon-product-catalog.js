    ...
    batch.forEach(value => {
      ...
      if (/^\d+$/.test(normalized)) {
        ...
        if (Number.isSafeInteger(numeric)) {
          ...
          skuIds.add(normalized);
        }
      }
    });

    const payload = {
      offer_id: Array.from(offerIds),
      product_id: Array.from(productIds),
      sku: Array.from(skuIds)
    };

    if (!payload.offer_id.length) {
      payload.offer_id = batch.map(value => normalizeSku(value)).filter(Boolean);
    }
...
function applyMetadataToItems(items, metadataMap) {
  ...
  return items.map(item => {
    ...
    const trimmedName = item?.product_name && String(item.product_name).trim()
      ? String(item.product_name).trim()
      : null;
    const normalizedSku = sku ? sku.replace(/\s+/g, '').toLowerCase() : '';
    let baseName = trimmedName;
    if (baseName) {
      const normalizedName = baseName.replace(/\s+/g, '').toLowerCase();
      if (normalizedSku && normalizedName === normalizedSku) {
        baseName = null;
      } else if (baseName === '商品') {
        baseName = null;
      }

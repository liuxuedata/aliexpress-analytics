...
async function syncLazadaAds({
  ...
  if (shouldSync) {
    ...
    const mappedCampaigns = (campaigns || [])
      .map(record => mapCampaign(record, { siteId: normalizedSiteId, platform }))
      .filter(Boolean)
      .map(row => ({ ...row, platform }));

    const mappedMetrics = (metrics || [])
      .map(record => mapMetric(record, { siteId: normalizedSiteId, platform }))
      .filter(Boolean)
      .map(row => ({ ...row, platform }));
    ...
    if (mappedMetrics.length) {
      mappedMetrics.forEach(metric => {
        metric.site_id = normalizedSiteId;
      });

...
| 速卖通 | Lazada/Shopee 规划入口 | `public/site-dashboard.html`（导航脚本自动生成入口） | 运营 / 产品 / 订单 / 广告（待启用） | ⏳ 规划中 | `/api/ae_query` 扩展 | `/api/ae_upsert` 扩展 | 通过 `site-configs` 与 `site-modules` 自动出现在顶栏 |
...
- `public/site-dashboard.html`：面向 Lazada/Shopee 等新站点的动态壳层，调用 `/api/site-configs` 与 `/api/site-modules/{siteId}` 自动装配模块占位与字段提示。【F:public/site-dashboard.html†L338-L752】
...
## 3. 应用层次结构
### 3.1 前端（展示层）
- 静态页面部署于 Vercel 的 `/public` 目录，使用共享导航脚本 `public/assets/site-nav.js` 维护跨站菜单，该脚本会在初始化阶段读取 `/api/site-configs` 动态注入 Lazada、Shopee 等新站点链接。【F:public/assets/site-nav.js†L111-L334】

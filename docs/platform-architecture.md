...
### 2.3 子站点导航规范
- **左侧分区**：所有站点页面在左侧导航固定呈现“运营分析”“产品分析”“订单中心”“广告中心”四个互不依赖的分区，分别挂载到独立的 hash/tab，确保切换不会共享前端状态。
- **全局设置**：库存中心与权限中心作为顶栏或全局设置入口，仅对具备相应角色（`inventory_manager`、`super_admin`、`permissions_admin`）的用户渲染；默认情况下非授权用户在 DOM 中也不生成这些入口。
- **站点差异化**：允许各站点在四个分区内加载各自的组件与 API 组合，例如亚马逊的“订单中心”可聚焦 ASIN 维度，而独立站的“广告中心”以渠道维度展开；文档中需标注任何站点特有字段或视图。
- **空状态策略**：当某站点暂未实现某分区（如 Temu 广告中心）时，左侧仍保留入口，内容区展示“建设中”占位，并给出预计上线时间或关联 roadmap 里程碑。
...
- **字段兼容**：统一运营接口中的字段均按“可选（nullable）”处理，每个站点需在文档中声明缺失字段的替代方案或默认值（如 Temu 暂无加购数据时返回 `null` 并在 `meta.missingFields` 标注），避免强制要求所有平台具备完全一致的指标。

...
| Lazada | Lazada 站点壳层 | `public/lazada.html` | 运营 / 产品 / 订单 / 广告（待启用） | ⏳ 规划中 | —（待实现） | —（待实现） | 导航脚本从 `site-configs` 注入入口，页面通过 `platform-page.js` 展示当前站点 |【F:public/lazada.html†L1-L88】【F:public/assets/platform-page.js†L1-L54】【F:public/assets/site-nav.js†L24-L309】
...
| Ozon | Product Report | `public/ozon-detail.html` | 运营 / 产品 / 订单 / 广告（均启用独立模块） | ✅ 运营数据（运营报表已上线） + ⏳ 订单/广告 API 对接 | `/api/ozon/stats`, `/api/ozon/orders`（规划）, `/api/ozon/ads`（规划） | `/api/ozon/import` | 订单与广告模块将直连 Ozon 官方 API，写入统一订单/广告表 |
...
- `public/ozon-detail.html` 系列：Ozon 指标与报表上传，`ozon-orders.html`、`ozon-advertising.html` 将通过后台服务直连 Ozon 官方订单/广告 API，并把结果写入统一订单/广告模型。
...
- Ozon 订单与广告模块通过 `ozon_api` 连接器直接访问官方 API，后端统一写入 `orders`、`order_items`、`ad_campaigns`、`ad_metrics_daily`，并在 `site_module_configs` 中标记 `has_data_source` 以解锁导航。
- Lazada OAuth 回调暴露在 `/api/lazada/oauth/callback`，依赖 `LAZADA_APP_KEY`、`LAZADA_APP_SECRET`、`LAZADA_REDIRECT_URI` 环境变量；回调仅返回授权码，真正的令牌交换需在服务端完成并落入安全密钥库。
...
- **数据来源**：平台 API（如 速卖通订单报表、亚马逊 SP-API、Ozon 官方 Orders API）及人工 Excel 导入。
...
- **平台差异**：Ozon 广告指标需通过官方 Advertising API 获取，映射字段在 `platform_metric_profiles` 中维护，若缺失将通过 `metadata.missingFields` 告知前端隐藏相应卡片。
...
- **Ozon 强化**：改造订单、广告模块使用官方 API 直连（`ozon_api` 数据源），后端定时同步订单头/明细与广告系列到统一表结构，并在 README/OpenAPI 中声明字段覆盖范围。

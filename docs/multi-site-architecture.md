...
- `GET /api/lazada/oauth/callback` - Lazada OAuth 回调入口，仅返回授权码/错误信息，后续令牌交换需在服务端完成，并依赖 `LAZADA_APP_KEY`、`LAZADA_APP_SECRET`、`LAZADA_REDIRECT_URI` 环境变量。
...
- `GET /api/ozon/orders`（规划中） - 直接调用 Ozon 官方 Orders API，同步订单头/明细并写入统一订单表，为 Ozon 站点的订单中心提供数据。
- `GET /api/ozon/ads`（规划中） - 对接 Ozon Advertising API，生成广告系列与日指标，供 Ozon 站点的广告中心使用。
